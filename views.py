...
class View:
  ...
  def agenda_listar_nao_confirmados():
    ...
    for horario in View.agenda_listar():
      if horario.get_confirmado() == False:
        r.append(horario)
    return r    
  ...
  def agenda_confirmar_agendamento(id, confirmado):
    id = int(id)
    confirmado = bool(confirmado)
    agenda = NAgenda.listar_id(id)
    View.agenda_atualizar(id, agenda.get_data(), confirmado, agenda.get_id_cliente(), agenda.get_id_servico())        
